...
* 02深入了解机器学习（20 分钟）什么是损失函数，权重weight和 bias 是什么
* 03降低损失（60 分钟）两种梯度下降，及对学习率的实验
* 04使用 TensorFlow 基本步骤（60 分钟）不能不懂的 TensorFlow
* 05泛化（15 分钟）什么是过拟合，怎样评价一个模型的好坏，把数据集分成测试和训练两部分
